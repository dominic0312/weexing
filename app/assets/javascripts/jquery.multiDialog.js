...
( function($, undefined) {
				...
				types : {
					...
					config : {
						image : {
							...
							template : "<img width='100%' height='100%' alt='{alt}' title='{title}' src='{path}'/><a href='#next' class='multibox-api next' rel='next'>选择</a><a href='#prev' class='multibox-api prev' rel='prev'>取消</a>",
							title : function(element) {
								return element.find("img").attr("alt") || element.text();
							},
							desc : function(element) {
								return element.find("img").attr("longdesc");
							},
							marker : {
								title : function(element) {
								...
								addr : function(element) {
									return element.find("img").attr("addr");
								}
		...
		$.extend(MultiDialog.prototype, {
			_create : function(_elements, _options) {
				...
				var that = this, options = this.options, elements = $(_elements);
			...
			openLink : function(_data) {
				var options = this.options, data = this._openLinkHelper(_data), typeGet, config, widthGet, heightGet, fn;
			...
			openImage : function(data) {
				...
				var that = this, options = this.options, image = new Image();
			...
			openVimeo : function(data) {
				data = this._openLinkHelper(data);
				var match = data.href.match(/http:\/\/(www\.)?vimeo.com\/(\d+)/), path = "http://player.vimeo.com/video/" + match[2] + this.options.types.config.vimeo.addParameters;
			...
			openAjax : function(data) {
				data = this._openLinkHelper(data);
				var that = this, options = this.options, ajaxOptions;
			...
			openGallery : function(group, index) {
				var that = this, groupIsJquery = group instanceof jQuery;
			...
			_createDialog : function(data) {
				...
				// create dialog
				this.uiDialog.dialog($.extend(true, {}, that.options.dialog, {
			...
			_addGalleryButtons : function() {
				var that = this, prevDisabled = !!(this.index === 0 && !this.options.gallery.loop ), nextDisabled = !!(this.index === this.group.length - 1 && !this.options.gallery.loop );
			...
			_changeGalleryButtons : function() {
				if (!this.options.gallery.loop && this.options.dialog.buttons) {
					var buttonpane = this.uiDialogWidget.children(".ui-dialog-buttonpane"), prev = buttonpane.find(".prev"), next = buttonpane.find(".next");
			...
			_addNonMouseControl : function() {
				...
				if ($.event.special.swipe) {
					this.uiDialogWidget.on("swipeleft." + this.widgetName, function(event) {
						that.next();
						event.preventDefault();
			...
			_defaultHandler : function(html, title, data) {
				var that = this, _data = $.extend({}, data, {
					html : html,
					title : title,
					desc : ""
				});
			...
			_getUrlVar : function(href, name) {
				var hashes = href.slice(href.indexOf("?") + 1).split("&"), vars = [], hash, i;
